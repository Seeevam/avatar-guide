...
const NationSelector = () => {
    return (
        <div className='nation-selector-container  px-6 '>

        <p className='nation-selector-text'>Select any nation</p>
        <div className='nation-container grid grid-cols-2  sm:flex sm:flex-row justify-center '>
        {
      ...
      ["air",'https://i.pinimg.com/originals/e6/79/49/e6794931f852802bfec52c8133d4f34d.png']]
            .map((name,i)=>
            <Link className={`element ${name[0]} mx-2 my-2 sm:mx-4 ` } to={`/${name[0]}`} key={i}>
                <img className= ' w-36 sm:w-60' src = {name[1]} alt = {name[0]} />
            </Link>
            )
        }</div>
        </div>
